import { GPSService } from './shared/services/gps/gps.service';
declare var com: any;
declare var android: any;

import { Accuracy } from 'tns-core-modules/ui/enums';
import { watchLocation, isEnabled, distance, clearWatch } from 'nativescript-geolocation';

import { LatLng } from './shared/models/latLng/latLng.class';
import {ReflectiveInjector} from '@angular/core';
import {Observable} from "rxjs";

// @TODO: delete debug comments

@JavaProxy("com.tns.GeoService")
export class GeoService extends com.pip3r4o.android.app.IntentService {

    private watchId;
    private gpsService;
    public permission$: Observable<boolean>;

    private injectGPSService(){

        if (this.gpsService) return;

        let injector = ReflectiveInjector.resolveAndCreate([
            GPSService
        ]);

        this.gpsService = injector.get(GPSService);
    }

    public GeoService(){
        this.super('GeoService');
    }

    public startWatchingLocation(){
        console.log('Watch Loction');

        if(this.watchId) return;

        console.log('Have no Watch Id');

        const OPTIONS = {
            desiredAccuracy: Accuracy.high,
            updateTime: 1000 * 60,
            minimumUpdateTime : 1000 * 20
        };

        this.watchId = watchLocation(response => {
            console.log('Watch location');
            if (response) {

                // @TODO: try get distance

                let { latitude, longitude } = response;
                let location = new LatLng(latitude, longitude);

                this.gpsService.sendLocation(location).subscribe(() => {

                }, (e) => {
                    console.log('Error in send Location');
                    console.log("Error: " + (e.message || e))
                    this.StopWatchingLocation();
                });
            }
        }, (e) => {
            console.log('Error in watch Location');
            console.log("Error: " + (e.message || e));
            this.StopWatchingLocation();
        }, OPTIONS);
    }

    public StopWatchingLocation(){
        if(this.watchId) {
            clearWatch(this.watchId);
            this.watchId = null;
        }
    }

    protected onHandleIntent(intent): void {

        const INTERVAL = 5000;

        this.injectGPSService();

        // @TODO: delete Observable, try change it to ApplicationEventData (on resume and suspend) as in settings component

        this.permission$ = Observable.interval(INTERVAL).flatMap(() => Observable.fromPromise(isEnabled()));

        this.permission$.subscribe(hasPermission => {
           if (hasPermission === true) {
               console.log('Have Permission, Start');
               this.startWatchingLocation();
           } else {
               console.log('No Permission, Stop');
               this.StopWatchingLocation();
           }
        }, (e) => {
            console.log('Error, Stop');
            console.log("Error: " + (e.message || e));
            this.StopWatchingLocation();
        }, () => {
            console.log('Complete, Stop');
            this.StopWatchingLocation();
        });


    }

}
